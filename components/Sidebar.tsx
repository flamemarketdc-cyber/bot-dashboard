...
const NavHeader: React.FC<{isExpanded: boolean; label: string}> = ({ isExpanded, label }) => (
    <h3 className={`text-xs font-bold tracking-wider text-slate-500 mt-4 mb-2 transition-opacity duration-100 ${isExpanded ? 'opacity-100 px-4' : 'opacity-0'}`}>
        {label}
    </h3>
)
...
const Sidebar: React.FC<SidebarProps> = ({ onRefresh }) => {
    ...
    const navSections = [
        {
            items: [
                ...
                { id: 'general', label: 'General Settings', icon: <CogIcon className="w-6 h-6 text-slate-300" /> },
    ...
    return (
        <aside 
            className={`transition-all duration-300 ease-in-out bg-[#16191C] border-r border-slate-700/40 p-3 flex-shrink-0 flex flex-col ${isExpanded ? 'w-64' : 'w-20'}`}
        ...
        >
            ...
            <button
                onClick={onRefresh}
                className="w-full flex items-center justify-center gap-2 px-2 h-10 mb-4 rounded-md text-sm font-semibold transition-all duration-200 text-slate-300 bg-slate-800/60 hover:bg-slate-700/80 hover:text-white"
            >
                <RefreshIcon />
